...
- [Architecture](#arc*itecture)
- [Visual architecture and execution guide](#visual-architecture-and-execu*ion-guide)
- [Repository map](#rep*sitory-map)
- [Prerequisites and setup](#prerequisit*s-and-setup)
- [Configuration](#*onfiguration)
- [Running the system](#runn*ng-the-system)
- [API reference]*#api-reference)
- [Data, storage, queues, and indexes](#data-storage-qu*ues-and-indexes)
- [Security and privacy](#sec*rity-and-privacy)
...
9. Conversation revocation is immediate. The retention worker asynchronously deletes associated records and artifacts and writes *esponse-safe audit events.

### Boundary rules

- The frontend never verifies identity itself; protected API calls c*rry `Authorization: Bearer <Firebase-ID-token>`.
- The API owns HTTP, Firebase/Firestore adapte*s, authorization, and orchestration entry points.
- The worker owns long-running OCR, rendering, queue pollin*, retries, retention, and local artifact adapters.
- `packages/document-core/python` contains provider-neutral rendering, privacy, and retention models/contracts. It has no FastAPI, Fi*estore, queue, renderer, or storage implementation.
...
Additional API security settings exist in `services/api/src/formwise_api/config.py`: CORS credential>/max age, HSTS, CSP, and readiness heartbeat settings. Their code defaults are authoritative where `.env.example` does not specify them.
...
The App Router has publ*c `/` and protected `/app` routes. `app-shell.tsx`, `app-header.tsx`, and `app-sidebar.tsx` provide the responsive permanent SaaS shel*. The pages cover home, upload/forms, OCR/privacy/understanding*assignments, conversation, history, and settings.

`AuthProvider` initializes Firebase once, enables local persistence, observes ID-token changes, and calls `/me`. Compon*nts consume the backend profile as the application source of truth. `next-intl` request configuration and `messages/en.json`, `hi*json`, and `te.json` supply the installed internationalization structure; the dashboard currently uses explanation keys a* placeholders rather than rendering localized policy text.

## Security and privacy

- Google-only Firebase Auth; API verifies every token with Firebase Admin and checks ownership before protected reads/writes/down*oads.
...
- Files are quarantined before OCR. Th* current scanner is an interface/fail-closed hook; no concrete antivirus engine is bundled.
- Privacy Engine findings are used to redact/quarantine sensitive data. `ASK_USER` never sends raw sensitive values to AI.
- Field Map v1 is immutable. Renderers consume only original artifact, Field Ma*, and approved assignments. Fillable PDFs dereference exactly the stored widget reference;*they do not enumerate or infer alternatives.
...
| Docker cannot share artifacts                    | Started services outside Compose volumes                              | Use Compose, which mounts matching named volumes to API and worker. For direct runs, configure all local storage variables as shared absolu