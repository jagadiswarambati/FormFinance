    ...
    <main className="min-h-screen bg-[#f5f7f8] text-slate-950 dark:bg-slate-950 dark:text-slate-50">
      <section className="mx-auto flex min-h-screen max-w-7xl flex-col justify-between px-6 py-8 sm:px-10 lg:px-16">
        ...
        <div className="grid gap-12 py-16 lg:grid-cols-[1.1fr_0.9fr] lg:items-center">
          <div><p className="text-sm font-semibold uppercase tracking-[0.2em] text-emerald-700">AI Finance Controller</p><h1 className="mt-6 max-w-3xl text-5xl font-semibold leading-[1.05] tracking-tight sm:text-6xl">Finance operations, verified automatically.</h1><p className="mt-6 max-w-xl text-lg leading-8 text-slate-600 dark:text-slate-300">Automated settlement verification, evidence matching, and exception handling.</p><div className="mt-10 flex flex-wrap gap-3"><Link className="rounded-md bg-emerald-700 px-6 py-3 text-sm font-semibold text-white transition hover:bg-emerald-800" href="/app">Login</Link><Link className="rounded-md border border-slate-300 bg-white px-6 py-3 text-sm font-semibold text-slate-800 transition hover:border-emerald-700 hover:text-emerald-800 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-100" href="/app">Sign Up</Link></div><p className="mt-5 text-xs text-slate-500">Demo access — authentication is disabled.</p></div>
